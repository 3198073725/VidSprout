...
def get_file_name(filename):
    # Windows 兼容：使用 os.path.basename 而不是 split("/")
...
def url_from_path(filename):
    ...
    media_root_normalized = settings.MEDIA_ROOT.replace("\\", "/")
    relative_path = filename_normalized.replace(media_root_normalized, "")
    # 确保路径以 / 开头
...
def media_file_info(input_file):
    ...
    # Windows 兼容：使用 Python 内置函数替代 Unix 命令
    try:
        ...
        if len(timestamps_list) > 1:
            ...
            with open(concat_list_path, "w") as f:
                for segment in segment_files:
                    # Windows 兼容：FFmpeg concat 需要 Unix 风格的路径（使用正斜杠）
