...
def kill_ffmpeg_process(filepath):
    ...
    if platform.system() == "Windows":
        ...
        try:
            # 使用 wmic 查找进程
            cmd = f'wmic process where "commandline like \'%{filepath}%\' and name=\'ffmpeg.exe\'" get processid'
            ...
            for pid in lines:
                if pid.isdigit():
                    ...
                    result = subprocess.run(kill_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
                    return result
        except Exception as e:
            logger.warning(f"Failed to kill ffmpeg process on Windows: {e}")
            return False
    else:
        # Unix/Linux 上使用原来的方法
