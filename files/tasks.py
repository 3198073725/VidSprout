...
def chunkize_media(self, friendly_token, profiles, force=True):
    ...
    file_name = helpers.get_file_name(media.media_file.path)  # Windows 兼容：使用 helpers.get_file_name
    ...
    to_profiles = []
    chunks_dict = {}
    # calculate once md5sums (Windows 兼容：使用 Python hashlib)
    import hashlib
    for chunk in chunks:
        try:
...
def whisper_transcribe(friendly_token, translate_to_english=False):
    ...
    with tempfile.TemporaryDirectory(dir=settings.TEMP_DIRECTORY) as tmpdirname:
        ...
        output_name = os.path.join(tmpdirname, subtitle_name)  # Windows 兼容：使用 os.path.join

        # Windows 兼容：使用 media.media_file.path 而不是硬编码的 Unix 路径
        media_file_path = media.media_file.path
        cmd = f"whisper {media_file_path} --model {settings.WHISPER_MODEL} --output_dir {tmpdirname}"
        if translate_to_english:
            cmd += " --task translate"
...
def produce_sprite_from_video(friendly_token):
    ...
    with tempfile.TemporaryDirectory(dir=settings.TEMP_DIRECTORY) as tmpdirname:
        try:
            # Windows 兼容：使用 os.path.join 而不是字符串拼接
            tmpdir_image_files = os.path.join(tmpdirname, "img%03d.jpg")
