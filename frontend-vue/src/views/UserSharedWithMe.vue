...
async function loadSharedWithMeMedia() {
  loading.value = true
  try {
    // 使用修正后的API：获取别人分享给我的媒体
    const response = await MediaAPI.getSharedWithMeMedia({ page: currentPage.value })
...
function getSharedByText(item: MediaItem) {
  return `由 ${(item as unknown as {shared_by?: string}).shared_by || item.author_name} 分享`
}

function getSharedDateText(item: MediaItem) {
  return formatDate((item as unknown as {shared_date?: string}).shared_date || item.add_date)
}

onMounted(loadSharedWithMeMedia)
</script>

<template>
  <div class="user-shared-with-me-container">
    <!-- 对应后端模板的头部信息 -->
    ...
    <div v-else-if="userExists">
      ...
      <div id="page-profile-media" class="shared-with-me-content">
        ...
        <div v-if="mediaList?.results?.length" class="media-list-section">
              ...
              <div class="media-info">
                ...
                <div class="media-meta">
                  <div class="meta-row">
                    ...
                    <span class="meta-value">{{ getSharedByText(item) }}</span>
                  </div>
                  
                  <div class="meta-row">
                    <span class="meta-label">分享时间:</span>
                    <span class="meta-value">{{ getSharedDateText(item) }}</span>
                  </div>
                  ...
                  <div class="meta-row">
                    ...
                    <span class="meta-value">{{ item.likes || 0 }} 个</span>
                  </div>
                </div>
                
                <div class="media-actions">
                  <el-button size="small" type="primary" @click="openMedia(item)">
                    立即观看
                  </el-button>
                  
                  <el-button 
                    size="small"
                    @click="router.push(`/user/${item.user}`)"
                  >
                    查看作者
                  </el-button>
          ...
          <div v-if="mediaList.count > pageSize" class="pagination-container">
            <el-pagination
              ...
              @current-change="loadSharedWithMeMedia"
            />
          </div>
        </div>
