...
class User(AbstractUser):
    ...
    description = models.TextField(_("关于我"), blank=True)

    name = models.CharField(_("姓名"), max_length=250, db_index=True)
    date_added = models.DateTimeField(_("添加日期"), default=timezone.now, db_index=True)
    is_featured = models.BooleanField(_("是否推荐"), default=False, db_index=True)
    is_approved = models.BooleanField(_("是否审核通过"), default=False, null=True, blank=True, db_index=True)

    title = models.CharField(_("职位"), max_length=250, blank=True)
    advancedUser = models.BooleanField(_("高级用户"), default=False, db_index=True)
    media_count = models.IntegerField(_("媒体数量"), default=0)  # save number of videos
    notification_on_comments = models.BooleanField(
        _("是否接收评论邮件通知"),
        default=True,
    )
    location = models.CharField(_("位置"), max_length=250, blank=True)
    is_editor = models.BooleanField(_("编辑员"), default=False, db_index=True)
    is_manager = models.BooleanField(_("管理员"), default=False, db_index=True)
    allow_contact = models.BooleanField(_("是否允许联系"), default=False)
